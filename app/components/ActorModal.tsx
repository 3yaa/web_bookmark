...
export default function ShowModal({
  ...
  const cols =
    selectedActor || castLoading ? 6 : Math.min(Math.max(cast.length, 1), 6);
  ...
  return (
    <div
      className={`fixed inset-0 bg-black/80 backdrop-blur-lg flex items-center justify-center z-20 p-4 transition-opacity duration-200 ${mounted ? "opacity-100" : "opacity-0"}`}
      onClick={onClose}
    >
      <div
        className={`relative w-full ${colMaxWidth[cols]} max-h-[88vh] flex flex-col overflow-hidden rounded-2xl bg-zinc-950 border border-zinc-800/50 shadow-2xl shadow-black/80 transition-all duration-300 ${mounted ? "opacity-100 translate-y-0 scale-100" : "opacity-0 translate-y-2 scale-[0.97]"}`}
        ...
        <div className="shrink-0 flex items-center justify-between px-6 py-4 border-b border-zinc-800/50 bg-linear-to-b from-zinc-900/50 to-transparent">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <p className="text-[10px] text-zinc-400/60 font-semibold uppercase tracking-[0.18em] mb-0.5">
                {selectedActor ? "Filmography" : "Featured Cast"}
              </p>
              <h2 className="text-zinc-200/90 text-lg font-semibold leading-tight truncate tracking-tight">
                {selectedActor ? selectedActor.name : mediaTitle}
              </h2>
            </div>
          </div>
                ...
                <div className="flex items-center gap-1 p-1 rounded-lg bg-linear-to-br from-zinc-900/80 to-zinc-950 border border-zinc-800/60 shadow-md shadow-black/40">
                  {(["all", "tv", "movie"] as const).map((f) => (
                    <button
                      ...
                      className={`cursor-pointer px-3 py-1 rounded-md text-[11px] uppercase tracking-[0.12em] font-semibold transition-all duration-200 ${
                        mediaFilter === f
                          ? "bg-zinc-700/70 text-zinc-100 shadow-sm"
                          : "text-zinc-500 hover:text-zinc-300"
                      }`}
                    >
                      {f === "all" ? "All" : f === "tv" ? "Series" : "Film"}
                    </button>
                  ))}
                </div>
                <div className="flex items-center gap-1 p-1 rounded-lg bg-linear-to-br from-zinc-900/80 to-zinc-950 border border-zinc-800/60 shadow-md shadow-black/40">
                  {(["popularity", "recent"] as const).map((s) => (
                    <button
                      key={s}
                      onClick={() => onFilmSortChange(s)}
                      className={`cursor-pointer px-3 py-1 rounded-md text-[11px] uppercase tracking-[0.12em] font-semibold transition-all duration-200 ${
                        filmSort === s
                          ? "bg-zinc-700/70 text-zinc-100 shadow-sm"
                          : "text-zinc-500 hover:text-zinc-300"
        ...
        <div className="overflow-y-auto px-6 py-6">
          ...
          {!selectedActor && (
            <>
              ...
              {!castLoading && cast.length > 0 && (
                <div className={`grid ${colGrid[cols]} gap-x-4 gap-y-5`}>
                  {cast.map((member) => (
                    <div
                      key={member.id}
                    ...
                    >
                      ...
                      <div className="px-3 py-2.5">
                        <p className="font-semibold text-zinc-200 text-[13px] leading-tight line-clamp-1">
                          {member.name}
                        </p>
                        {member.character && (
                          <p className="text-zinc-400 text-[12px] mt-0.5 line-clamp-1 font-medium">
                            {member.character}
                          </p>
                        )}
                      </div>
              ...
              {!actorLoading && sortedWorks.length > 0 && (
                <>
                  <div className="grid grid-cols-5 gap-x-4 gap-y-5">
                    {sortedWorks.filter((w) => mediaFilter === "all" || w.media_type === mediaFilter).map((work) => {
                      const inList =
                        work.media_type === "tv" &&
                        addedShowIds?.has(String(work.id));
                      return (
                        <div
                          key={`${work.media_type}-${work.id}`}
                          onClick={() => onWorkClick?.(work)}
                          className={`bg-linear-to-b from-zinc-900/70 to-zinc-950/85 rounded-xl border shadow-md shadow-black/50 overflow-hidden ${onWorkClick ? "cursor-pointer hover:-translate-y-0.5 hover:shadow-xl hover:shadow-black/60 transition-all duration-300 ease-out" : ""} ${inList ? "border-emerald-800/70 border-2 hover:border-emerald-700" : "border-zinc-800/50 hover:border-zinc-700/60"}`}
                        >
                          <div className="relative aspect-2/3 bg-zinc-900 overflow-hidden">
                            {inList && (
                              <div className="absolute top-2 right-2 z-10 px-1.5 py-0.5 rounded-md bg-zinc-900/80 backdrop-blur-sm border border-zinc-700/60 text-[10px] font-semibold uppercase tracking-[0.12em] text-zinc-300">
                                In List
                              </div>
                            )}
                            {work.poster_path ? (
                          ...
                          <div className="px-2.5 pt-2.5">
                            <p className="font-semibold text-zinc-200 text-[12px] leading-snug line-clamp-1">
                              {work.title}
                            </p>
                            <div className="flex items-center justify-between mt-1">
                              <span className="text-zinc-500 text-[11px] font-medium">
                                {work.media_type === "tv" ? "Series" : "Film"}
                              </span>
                              {work.date && (
                                <span className="text-zinc-400 text-[11px] font-semibold tabular-nums">
                                  {work.date.slice(0, 4)}
                                </span>
                              )}
                            </div>
