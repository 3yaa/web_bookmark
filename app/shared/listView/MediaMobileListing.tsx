...
export function MediaMobileListing<T extends BaseMediaProps>({
  ...
  const sortOptions = [
    ...
    { label: differentColumns[0].label, sortKey: differentColumns[0].sortKey },
    { label: differentColumns[1].label, sortKey: differentColumns[1].sortKey },
    ...
    <div className="w-full mx-auto tracking-tight ">
      ...
      {!isProcessing && mediaItems.length === 0 && (
        <div className="text-center py-12">
          <p className="text-zinc-500 italic text-lg">{emptyListText}</p>
        </div>
      )}
      {/* LISTING */}
      {!isProcessing && mediaItems.length > 0 && (
